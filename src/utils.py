...
def process_control():
    data_shape = {
        ...
        "MNIST": [1, 28, 28],
        "CIFAR10": [3, 32, 32],
        "ModelNet40": [3, 32, 32, 12],
        "ShapeNet55": [3, 32, 32, 12],
        "MIMICL": [22],
        "MIMICM": [22],
    }
    ...
    cfg["local_epoch"] = (
        int(cfg["control"]["local_epoch"])
        if cfg["control"]["local_epoch"] != "none"
        else "none"
    )
    cfg["global_epoch"] = (
        int(cfg["control"]["global_epoch"])
        if cfg["control"]["global_epoch"] != "none"
        else "none"
    )
    ...
    cfg["attack"] = (
        None
        if cfg["attack"] == "None" or cfg["attack"] == "none" or cfg["attack"] == "null"
        else cfg["attack"]
    )
...
def make_optimizer(model, tag):
    if cfg[tag]["optimizer_name"] == "SGD":
        ...
        optimizer = optim.Adam(
            model.parameters(), lr=cfg[tag]["lr"], weight_decay=cfg[tag]["weight_decay"]
        )
...
def make_scheduler(optimizer, tag):
    if cfg[tag]["scheduler_name"] == "None":
        ...
        scheduler = optim.lr_scheduler.StepLR(
            optimizer, step_size=cfg[tag]["step_size"], gamma=cfg[tag]["factor"]
        )
    elif cfg[tag]["scheduler_name"] == "MultiStepLR":
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=cfg[tag]["milestones"], gamma=cfg[tag]["factor"]
        )
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=cfg[tag]["num_epochs"]["global"],
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode="min",
            factor=cfg[tag]["factor"],
            patience=cfg[tag]["patience"],
            verbose=False,
            threshold=cfg[tag]["threshold"],
            threshold_mode="rel",
            min_lr=cfg[tag]["min_lr"],
        )
    elif cfg[tag]["scheduler_name"] == "CyclicLR":
        scheduler = optim.lr_scheduler.CyclicLR(
            optimizer, base_lr=cfg[tag]["lr"], max_lr=10 * cfg[tag]["lr"]
        )
    else:
...
def resume(model_tag, load_tag="checkpoint", verbose=True):
    if os.path.exists("./output/model/{}_{}.pt".format(model_tag, load_tag)):
        ...
        logger_path = "output/runs/train_{}_{}".format(
            cfg["model_tag"], datetime.now().strftime("%b%d_%H-%M-%S")
        )
...
def collate(input):
    for k in input:
        if cfg["data_name"] in ["MIMICL", "MIMICM"]:
            if k == "data":
                input[k] = pad_sequence(
                    input["data"], batch_first=True, padding_value=0
                )
            elif k == "target":
                input[k] = pad_sequence(
                    input["target"], batch_first=True, padding_value=-65535
                )
            else:
...
def plot_classes_preds(net: Conv, images, labels):
    ...
    for idx in range(4):
        ...
        ax.set_title(
            "{0}, {1:.1f}%\n(label: {2})".format(
                pred_class_name, pred_prob, true_class_name
            ),
...
def plot_output_preds_target(
    ...
    for i, ax in enumerate(axes.flat):
        ...
        ax.set_title(
            "{0}, {1:.1f}%\n(label: {2})\n(target: {3})".format(
                pred_class_name, pred_prob, true_class_name, target_class_name
            ),
            color=("green" if pred_class_name == true_class_name else "red"),
        )
    plt.tight_layout()
...
def print_classes_preds(labels: torch.Tensor, output: torch.Tensor):
    ...
    for i in range(4):
        ...
        print(
            f"label {i}: {labels[i].item()}, type: {type(labels[i].item())} class_name {true_class_name}"
        )
        print(
            f"pred {i}: {pred[i]}, type: {type(pred[i])} class_name {pred_class_name}"
        )
...
def evaluate_predictions(true_labels, predicted_probs, threshold=0.5):
    ...
    precision = precision_score(
        true_labels.numpy(), predicted_labels.numpy(), average="macro"
    )
    recall = recall_score(
        true_labels.numpy(), predicted_labels.numpy(), average="macro"
    )
...
def show_image_with_two_labels(image_tensor, true_label, target_label):
    ...
    plt.title(
        "True Label: {}\nTarget Label: {}".format(
            classes[true_label.item()], classes[target_label.item()]
        )
    )
...
def add_watermark_to_test_dataset(mark, dataset, keep_org):
    ...
    for i, sample in enumerate(dataset["test"]):
        altered_data, altered_target = add_watermark(
            mark=mark, data=(sample["data"], sample["target"]), keep_org=keep_org
        )
...
def numpy_to_torch(images_np):
    ...
    if (
        images_tensor.ndimension() == 4
    ):  # assuming a batch of images with shape (N, H, W, C)
...
def torch_to_numpy(images_tensor):
    ...
    if images_tensor.ndimension() == 4 and images_tensor.shape[1] in {
        1,
        3,
    }:  # assuming a batch of images with shape (N, C, H, W)
