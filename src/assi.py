...
class Assi:
    ...
    def reset(self):
        self.organization_output = [
            {split: None for split in cfg["data_size"]}
            for _ in range(cfg["global"]["num_epochs"] + 1)
        ]
        self.organization_target = [
            {split: None for split in cfg["data_size"]}
            for _ in range(cfg["global"]["num_epochs"] + 1)
        ]
        self.organization_mal_target = [
            {split: None for split in cfg["data_size"]}
            for _ in range(cfg["global"]["num_epochs"] + 1)
        ]
        return
    ...
    def make_model_name(self):
        ...
        for i in range(len(model_name)):
            model_name[i] = [
                model_name[i] for _ in range(cfg["global"]["num_epochs"] + 1)
            ]
        return model_name
    ...
    def make_organization(self, poison_agent=None):
        ...
        for i in range(len(feature_split)):
            ...
            if i < (cfg["num_users"] - cfg["num_attackers"]):
                ...
                organization[i] = MalOrg(
                    organization_id=i,
                    feature_split=feature_split_i,
    ...
    def broadcast(self, dataset, epoch):
        ...
        for split in dataset:
            ...
            loss = models.loss_fn(
                self.organization_output[epoch - 1][split],
                self.organization_target[0][split],
                reduction="sum",
            )
            loss.backward()
            ...
            self.organization_target[epoch][split] = -copy.deepcopy(
                self.organization_output[epoch - 1][split].grad
            )
                ...
                if "dl" in cfg and cfg["dl"] == "1":
                    ...
                    if epoch == 1:
                        ...
                        dataset[split].target = [
                            np.concatenate(
                                [dataset[split].target[i], target[i]], axis=1
                            )
                            for i in range(len(dataset[split].target))
                        ]
                else:
                    target = self.organization_target[epoch][split].numpy()
                    if "pl" in cfg and cfg["pl"] != "none":
                        target = make_privacy(target, cfg["pl_mode"], cfg["pl_param"])
                    dataset[split].target = np.split(
                        target, np.cumsum(dataset[split].length), axis=0
                    )
            else:
                if "dl" in cfg and cfg["dl"] == "1":
                    target = self.organization_target[epoch][split].unsqueeze(1).numpy()
                    ...
                    if epoch == 1:
                        ...
                        dataset[split].target = np.concatenate(
                            [dataset[split].target, target], axis=1
                        )
                else:
                    target = self.organization_target[epoch][split].numpy()
    ...
    def update(self, organization_outputs, anomalies_by_org, epoch):
        if cfg["assist_mode"] == "none":
            ...
            for split in organization_outputs[0]:
                ...
                _organization_outputs[split] = torch.stack(
                    _organization_outputs[split], dim=-1
                )

        if cfg["assist_mode"] == "bag":
            for split in organization_outputs[0]:
                if not cfg["defense"] == None and split == "test":
                    _organization_outputs[split] = remove_anomalies(
                        _organization_outputs[split], anomalies_by_org
                    )
                self.organization_output[epoch][split] = _organization_outputs[
                    split
                ].mean(dim=-1)
            ...
            with torch.no_grad():
                ...
                print(
                    f"self.assist_parameters epoch {epoch}: {self.assist_parameters[epoch]}"
                )
                model.train(False)
                for split in organization_outputs[0]:
                    if not cfg["defense"] == None and split == "test":
                    ...
                    input = to_device(input, cfg["device"])
                    self.organization_output[epoch][split] = model(input)[
                        "target"
                    ].cpu()
        else:
        ...
        if "train" in organization_outputs[0]:
            if cfg["assist_rate_mode"] == "search":
                input = {
                    "history": self.organization_output[epoch - 1]["train"],
        ...
        with torch.no_grad():
            for split in organization_outputs[0]:
                self.organization_output[epoch][split] = (
                    self.organization_output[epoch - 1][split]
                    + self.assist_rates[epoch] * self.organization_output[epoch][split]
                )
        return
    ...
    def update_al(self, organization_outputs, iter):
        ...
        if "train" in organization_outputs[0]:
            if cfg["assist_rate_mode"] == "search":
                input = {
                    "history": self.organization_output[iter - 1]["train"],
        ...
        with torch.no_grad():
            for split in organization_outputs[0]:
                self.organization_output[iter][split] = (
                    self.organization_output[iter - 1][split]
                    + self.assist_rates[iter] * self.organization_output[iter][split]
                )
        return
